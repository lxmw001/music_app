...
class GeminiService {
  ...
  Future<SongMetadata?> getSongMetadata(String videoTitle) async {
    ...
    if (!_hasApiKey) {
      rlog('[Gemini] no API key, using regex for: "$videoTitle"');
      return _regexFallback(videoTitle);
    }

    // 3. Call Gemini
    rlog('[Gemini] calling API for: "$videoTitle"');
    try {
...
Title: "$videoTitle"'''
      ...
      if (response.statusCode != 200) {
        rlog('[Gemini] API error ${response.statusCode}');
        return null;
      }
      ...
      try {
        ...
        rlog('[Gemini] artist=${metadata.artist}, genre=${metadata.genre}, isMix=${metadata.isMix}');
        rlog('[Gemini] queries=${metadata.suggestedQueries}');
        await _saveToCache(key, metadata);
        return metadata;
      } catch (e) {
        rlog('[Gemini] JSON parse error: $e\nRaw: $cleaned');
        return null;
      }
    } catch (e) {
      rlog('[Gemini] exception: $e');
      return null;
    }
  }
